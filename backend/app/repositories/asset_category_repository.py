...
Purpose
-------
Encapsulates all direct PostgreSQL access for the AssetCategory entity (and closely related child rows).
...
- services/*.py -> the only layer permitted to call AssetCategoryRepository directly.
- db/models/*.py -> operates on the AssetCategory ORM model (and related models where noted).
...
class AssetCategoryRepository(BaseRepository):
    """
    Repository for the AssetCategory entity.
